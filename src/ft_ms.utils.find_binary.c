	...
	)
{
	...
	if (dir)
	{
		...
		{
			if (ftstr__search_start(bin_name, dp->d_name, param) > -1)
			{
...
{
	fts__clear(buffer);
	if (MS__FIND_BINARY_SEARCH == mode)
	{
		fts__add(buffer, directory_path);
		fts__add(buffer, "/");
	}
	if (search_in_directory(directory_path,
			binary_name,
...
{
	if ('\0' == (*path_str = ms__find_env_key(env, "PATH"))[0]
		|| NULL == (*paths = ms__parse_str(*path_str, ":")))
	{
		ft_printf(MS__NAME"PATH not set\n");
		return (-1);
	}
...
char *ms__find_binary(t_array *env, char *binary_name, t_s *buffer, int mode)
{
	...
	if (OK != init(env, &path_str, &paths))
		return (NULL);
	while ((directory = ftarray__next(paths)))
	{
		if (search_path(binary_name, *directory, buffer, mode))
			break;
	}
	ftarray__free_func(&paths, ftarray__free_func_str, NULL);
	return (buffer->data[0] ? buffer->data : NULL);
}
